...
[Back to the EZ setup](../README.md) · [Validation record](../VALIDATION.md)

Run all commands below from the repository folder. Quit the app before rebuilding
or starting another instance. For everyday use, launch the packaged app through
Finder as described in the EZ setup.

This repository preserves the original 2023 `main.c` experiments and adds a new,
direct HID backend in `src/hid_guard.m` for the League of Legends Mac cursor escape
...
remain unverified. See [VALIDATION.md](../VALIDATION.md) for the observations and limits.
...
could therefore reach the desktop before the correction. The preserved polling
version in `main.c` also checks only the main display's horizontal limits, not all four edges
...
Quit MouseLock using its menu first. In your existing repository folder, run:

```sh
git switch main && git pull --ff-only
```

If Git reports local changes or a divergent branch, stop and preserve your work;
do not force-reset it. After a successful update, rebuild the same app location:

```sh
python3 build_app.py "$HOME/Applications/MouseLock HID.app"
```

Use your original output path instead if you installed elsewhere. Relaunch via
Finder or `open -a` and renew this app's permissions if needed, as described
above. Do not keep multiple copies with the same bundle identifier.
